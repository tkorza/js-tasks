...
let arrMail: string[] = ['ttkk@mail.ru', 'ttkk@mail.ru', 'gggggg@mail.ru', 'ttttttt@mail.ru', 'aaaaaaa@mail.ru']
let resultArray: string[] = []
arrMail.forEach((el: string) => {
    if ((/(^\w+\@[a-zA-Z]+\.[a-zA-Z]{2,6})/gm).test(el) && !resultArray.includes(el)) resultArray.push(el)
})
